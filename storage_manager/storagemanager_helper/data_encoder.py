...
class DataEncoder:
    ...
    def encode_varchar(self, value, max_length):
        ...
        return struct.pack("<I", length) + padded
    ...
    def decode_varchar(self, byte_data, offset, max_length):
        length = struct.unpack_from("<I", byte_data, offset)[0]
        offset += 4
        raw_bytes = struct.unpack_from(f"{max_length}s", byte_data, offset)[0]
        value = raw_bytes[:length].decode('utf-8')

        return value, offset + max_length
